...
`Program.Main` initializes `VelopackApp` before WPF. Velopack's default startup behavior applies an already staged update on the next launch; users can instead choose **Restart now** to apply it immediately. `UpdateCoordinator` owns the platform-neutral daily-check, availability, user-initiated download, progress, and restart states. It persists time only after a successful check and maps offline, GitHub rate-limit, checksum, update-lock, and unpackaged-development failures to compact messages. `VelopackUpdateClient` is the production adapter for public GitHub Releases. The release-candidate build includes GitHub prereleases; this must be changed to stable-only when the first stable release is approved.
